    ...
    <div class="container mx-auto flex flex-row flex-wrap justify-evenly flex-1 items-center text-white">
        <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
            ...
            <div>
                ...
                <select @if($candidats->count() == 0) disabled @endif name="candidat_id" id="candidat" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-logo-orange-500 focus:border-logo-orange-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                    @forelse ($candidats as $candidat)
                        <option value="{{ $candidat->id }}">{{ $candidat->fullnameage }}</option>
                    @empty
                        <option value disabled> Pas de candidat </option>
                    @endforelse 
                </select>
            </div>
            <div>
