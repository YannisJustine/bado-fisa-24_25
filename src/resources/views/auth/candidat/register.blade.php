    ...
    <div class="flex h-screen flex-col justify-center px-6 py-12 lg:px-8 bg-center bg-no-repeat bg-gray-200 bg-blend-multiply bg-cover text-white" style="background-image: url('{{ Vite::asset('resources/images/background_login.jpg') }}')">
        <div class="mt-10 sm:mx-auto sm:w-full bg-gray-800  sm:max-w-lg sm:p-5 bg-opacity-90">
            <div class="sm:mx-auto sm:w-full sm:max-w-sm">
                ...
                <h2 class="mt-10 text-center text-2xl font-bold leading-9 tracking-tight text-shadow-sm shadow-black">Se créer un compte
                </h2>
            </div>
            <form class="space-y-6" action="{{ route('register.candidat.post') }}" method="POST">
                ...
                <div class="relative">
                    <label for="birthday" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-logo-orange-600 peer-focus:dark:text-logo-orange-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Date de naissance</label>
                ...
                <x-input type="password" name="password_confirmation" label="Confirmer le mot de passe" />
               
                <div class="mt-5">
                    <button type="submit"
                        class="flex w-full justify-center rounded-md bg-logo-orange-600 px-3 py-1.5 text-sm font-semibold leading-6 text-white shadow-sm ">Créer</button>
                </div>
            </form>

            <p class="mt-10 text-center text-sm text-white">
                Déjà membre ?
                <a href="{{ route('login.candidat') }}" class="font-semibold leading-6 text-logo-orange-400 hover:text-logo-orange-600">
                    Se connecter
                </a>
            </p>
        </div>
