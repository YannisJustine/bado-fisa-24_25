...
@section('body')
    <div class="flex h-screen flex-col justify-center px-6 py-12 lg:px-8 bg-center bg-no-repeat bg-gray-200 bg-blend-multiply bg-cover text-white" style="background-image: url('{{ Vite::asset('resources/images/background_login.jpg') }}')">
        <div class="mt-10 sm:mx-auto sm:w-full bg-gray-800  sm:max-w-lg sm:p-5 bg-opacity-90">
            ...
            <form class="space-y-6" action="{{ route('login.post.candidat') }}" method="POST">
                @csrf
                @if($errors->any())
                ...
                <div>
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <label for="email" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-logo-orange-600 peer-focus:dark:text-logo-orange-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Email</label>
                    </div>
                </div>
                <div>
                    <div class="relative z-0 w-full mb-5 group">
