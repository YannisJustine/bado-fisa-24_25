    ...
    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div class="relative flex h-16 items-center justify-between">
            ...
            <div class="absolute right-0 ">
                @auth
                    <div class="relative ml-3">
                        <div>
                            <button type="button"
                                ...
                                <div class="relative inline-flex items-center justify-center w-10 h-10 overflow-hidden bg-orange-100 rounded-full dark:bg-orange-600">
                                    <span class="font-medium text-orange-600 dark:text-white">{{ Auth::user()->initials }}</span>
                                </div>
                            </button>
                        </div>
                        ...
                        <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-orange-100 rounded-lg shadow dark:bg-orange-700 dark:divide-orange-600"
                            id="user-dropdown">
                            <div class="px-4 py-3">
                                <span class="block text-sm text-orange-900 dark:text-white">{{ Auth::user()->fullname }}</span>
                                <span
                                    class="block text-sm  text-orange-500 truncate dark:text-gray-100">{{ Auth::user()->email }}</span>
                            </div>
                ...
                @else
                    <x-link-button href="{{ route('login') }}" class="md:py-2 md:px-3 px-1 py-1 bg-orange-600 dark:hover:bg-orange-800 dark:hover:text-white">
                        Se connecter
                    </x-link-button>
                @endauth
            </div>
